...
class MoCo(nn.Module):
    ...
    def forward(self, input_s1_v1, input_s2_v1, input_s1_v2, input_s2_v2):
        ...
        # negative logits: NxK
        # use  intra-skeleton contrast
        # query-s1  negative-keys-s2
        l_neg_seq = torch.einsum('nc,ck->nk', [q, self.Queue_S2.clone().detach()])

        # query-s2  negative-keys-s1
        l_neg_graph = torch.einsum('nc,ck->nk', [r, self.Queue_S1.clone().detach()])
